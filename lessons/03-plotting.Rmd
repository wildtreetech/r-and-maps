...
ggplot(data = gapminder,
  aes(x=gdpPercap, y=lifeExp, size=pop, shape=continent, color=country, group=continent)) +
...
This works because:
```{r}
ggplot(data = gapminder[gapminder$continent=='Europe',], aes(x=gdpPercap, y=lifeExp, size=pop, shape=continent, color=country, group=continent)) + geom_point(alpha=0.5) + scale_x_log10() + geom_smooth(method='lm', aes(colour=continent)) + theme(legend.position = 'none')
```
